...
export default class VictoryScene extends Phaser.Scene {
    ...
    create() {
        ...
        const epilogue = [
            'The anomaly wave is contained.',
            'OCI Defense Grid is stable.',
            '',
            'API Gateway held the edge.',
            'Load Balancers shifted traffic.',
            'Compute fleets stayed online.',
            '',
            'Streaming captured every event.',
            'Analytics confirmed recovery.',
            'AI marked the region clear.',
            '',
            'Mission complete.'
        ];
        ...
        epilogue.forEach((line, i) => {
            this.time.delayedCall(i * 400, () => {
                const text = this.add.text(width / 2, 80 + i * 22, line, {
                    ...
                    fontSize: '14px',
                    fill: line === 'Mission complete.' ? '#ffdd00' : '#aaaaaa',
                    stroke: '#000000',
                    strokeThickness: 2
        ...
        localStorage.setItem('bestLevel', '5');
